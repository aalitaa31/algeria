...
module.exports = {
    ...
    onRequest: async function (member) {
        ...
        if(acarkalkmazban && acarkalkmazban.some(id => `k${member.user.id}` === id)) {
          try {
            await member.send(`**${member.guild.name}** sunucusundan kalıcı olarak bot tarafından yasaklandınız, Kalıcı banınız Sadece Rusyalı Tarafından açılabilir.`)
            member.ban({reason: 'Rusyalı Bot kalkmazban sistemi ile banlandı!'})
