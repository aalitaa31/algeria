...
module.exports = {
    Isim: "kadin",
    Komut: ["k", "kiz"],
    Kullanim: "kadin @rusyalı/ID <isim> <yaş>",
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if(!uye) return message.channel.send(`Hata: Lütfen bir üye etiketleyin veya Id giriniz!  __Örn:__  \`${client.sistem.a_Prefix}kadın @rusyalı/ID isim yaş\``).then(sil => sil.delete({timeout: 5000}));
    ...
    if(!isim || !yaş) return message.channel.send(`Hata: Lütfen tüm argümanları doldurunuz!  __Örn:__  \`${client.sistem.a_Prefix}kadın @rusyalı/ID isim yaş\``).then(sil => sil.delete({timeout: 5000}));
        ...
        uye.setNickname(`${BelirlenenIsim}`).catch();
            ...
            let kadın = uye.roles.cache.filter(x => x.managed).map(x => x.id).concat(acar.kadinRolleri)
