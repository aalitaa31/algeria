...
module.exports = {
    Isim: "ban",
    Komut: ["yasakla"],
    Kullanim: "yasakla @rusyalı/ID <sebep>",
  ...
  if (args[0] && (args[0].includes('bilgi') || args[0].includes('info'))) {
    if(!args[1] || isNaN(args[1])) return message.channel.send(`Hata: Lütfen geçerli yasaklanmış üye Id girin!  __Örn:__  \`${client.sistem.a_Prefix}ban bilgi @acar/ID\``).then(x => x.delete({timeout: 5000}));;
  ...
  if (!reason) return message.channel.send(`Hata: Lütfen bir sebep giriniz!  __Örn:__  \`${client.sistem.a_Prefix}ban @acar/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
  let ceza = {
    No: cezano,
    Cezalanan: victim.id,
  ...
  if (!victim) {
    ...
    if(kisi) {
      ...
      message.channel.send(`Hata: Lütfen bir üye veya sebep giriniz!  __Örn:__  \`${client.sistem.a_Prefix}ban @Rusyalı/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
    };
    return message.reply(`Hata: Lütfen bir üye veya sebep giriniz!  __Örn:__  \`${client.sistem.a_Prefix}ban @Rusyalı/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
  };
