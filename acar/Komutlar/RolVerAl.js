...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
   ...
   let kullanici = message.mentions.users.first() || message.guild.members.cache.get(args[1])
    ...
    if(args[0] !== "ver" && args[0] !== "al") return message.channel.send(`Hata: Bir üyeye rol verip almak için lütfen __argümanları__ doldurun Örn: \`.rol ver/al @rusyalı/ID <EtiketRol/RolID>\``).then(x => x.delete({timeout: 5000}));
    if(!kullanici) return message.channel.send('Hata: Bir üyeye rol verip almak için lütfen __argümanları__ doldurun Örn: \`.rol ver/al @rusyalı/ID <EtiketRol/RolID>\`').then(x => x.delete({timeout: 5000}));
    if(!rol) return message.channel.send('Hata: Bir üyeye rol verip almak için lütfen __argümanları__ doldurun Örn: \`.rol ver/al @rusyalı/ID <EtiketRol/RolID>\`').then(x => x.delete({timeout: 5000}));
