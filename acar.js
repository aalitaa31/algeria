...
client.modüller = {}; 
client.altbaslik = client.veri.Tag + " " + client.veri.sunucuUfakIsim + " Developer by Rusyalı"
...
fs.readdir("./acar/Komutlar", (err, files) => {
    ...
    console.log(`[ ${files.length} ] Adet Rusyalı Komutları Yüklenecek!`);
    files.forEach(file => {
  ...
  fs.readdir("./acar/Etkinlikler/", (err, files) => {
    ...
    files.forEach(fileName => {
      ...
      console.log(
        `[~ Rusyalı~ ] (${fileName}) isimli etkinlik yüklendi.`
      );
    });
  });
...
if(message.author.bot || message.channel.type === "dm") return;
  ...
  .setThumbnail(message.author.avatarURL())
  .setFooter(`Rusyalı Bilgilendirme  • bügün saat ${message.createdAt.getHours()+3}:${message.createdAt.getMinutes()}`, `${client.user.displayAvatarURL()}`)
...
if(newMessage.author.bot || newMessage.channel.type === "dm") return;
  ...
  .setFooter(`Rusyalı Bilgilendirme  • bügün saat ${newMessage.createdAt.getHours()+3}:${newMessage.createdAt.getMinutes()}`, `${client.user.displayAvatarURL()}`)
 ...
 client.on("message", (message) => {
    ...
    if(["tag", ".tag", "!tag"].includes(message.content.toLowerCase())){ 
...
client.login(client.sistem.a_Token).catch(err => console.error("[~ Lan Rusyalı ~] Discord API Botun tokenini doğrulayamadı."));
